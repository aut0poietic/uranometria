// Progress goes to stderr (ephemeral), reports to stdout, so `npm run parse > log`
// keeps the report and drops the bars.
...
// Pad only, never truncate: a clipped filename is worse than a ragged column.
export const NAME_WIDTH = 15;
export const FILE_WIDTH = 28; // longest is stellarium_modern_index.json
...
/**
 * Bar when `total` is known and stderr is a TTY; byte counter when it isn't
 * (chunked TAP); silent when piped. `total` is WIRE bytes.
 */
export function meter(label, total) {
  ...
  let read = 0;
  let last = 0;
  // Transparent decompression can overrun the total; drop to a counter rather
  // than render 130%.
...
/** NAME_WIDTH is what aligns these across separate parser processes. */
