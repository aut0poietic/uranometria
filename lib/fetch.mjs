// Verifies every file against its config sha256 both on fetch and on read, so a
// parse can never run against a source that drifted under its pin.

import { createHash } from 'node:crypto';
import { existsSync, mkdirSync, readFileSync, writeFileSync } from 'node:fs';
import { gunzipSync } from 'node:zlib';
import { paths, sources, curationFile, extrasFile } from '../config.mjs';
import { bytes, dim, FILE_WIDTH, green, meter, red } from './ui.mjs';

const sha256 = (buf) => createHash('sha256').update(buf).digest('hex');

/**
 * Drain a body, metering bytes as they arrive. Prefers the pinned `bytes`:
 * content-length lies under gzip transport encoding and is absent on chunked TAP.
 */
...
export async function fetchSource(key) {
  ...
  // The gunzipped source metered its compressed payload; say so or this line
  // looks like it disagrees with the bar.
...
export async function fetchAll() {
  // Sequential: one bar at a time, and polite to the upstream CDNs.
...
export const readExtras = () =>
  JSON.parse(readFileSync(new URL(extrasFile, paths.curation), 'utf8'));
